...
class Assignment4A:
    ...
    def fit(self, f: callable, a: float, b: float, d: int, maxtime: float) -> callable:
        ...
        # result = create_polynomial_interpolation_newton(points.T)

        return result

    def linear_least_squares(self, samples, d):
        A = numpy.vander(samples[0], d, increasing=True)
        y = samples[1]
        c = np.linalg.inv(A.T.dot(A)).dot(A.T.dot(y))
        poly = np.polynomial.Polynomial(c)
        return poly
...
class TestAssignment4(unittest.TestCase):
    ...
    def test_delay(self):
        f = DELAYED(0.1)(NOISY(0.01)(poly(1, 1, 1)))
